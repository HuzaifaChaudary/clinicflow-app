...
import { LayoutDashboard, Calendar, Users, FileText, Phone, FolderOpen, Settings, Pin, PinOff, ClipboardList, MessageSquare, FolderInput } from 'lucide-react';
...
export function RoleBasedSidebar({ activePage = 'dashboard', onPageChange }: SidebarProps) {
  ...
  const adminNav = [
    ...
    { id: 'intake-forms', label: 'Intake & Forms', icon: FolderInput },
  ...
  const doctorNav = [
    ...
    { id: 'voice', label: 'Voice Activity', icon: MessageSquare },
  ];

  const primaryNav = role === 'admin' ? adminNav : doctorNav;
  ...
  return (
    <aside
      ref={sidebarRef}
      className={`glass flex flex-col h-screen motion-state border-r ${
    ...
    >
      ...
      >
        {isExpanded ? (
          <>
            <div>
              <h1 style={{ color: 'var(--text-primary)' }}>Clinicflow</h1>
